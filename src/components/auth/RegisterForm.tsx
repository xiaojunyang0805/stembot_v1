...
export function RegisterForm({ className }: RegisterFormProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const roleMapping = {
        ...
        'educator': 'educator' as const,
        'parent': 'student' as const // Map parent to student role in auth system
      };
  ...
  const getBorderColor = (fieldName: keyof FormErrors, isFocused: boolean = false): string => {
    ...
    if (touchedFields[fieldName as keyof TouchedFields] && errors[fieldName]) return '#dc2626';
