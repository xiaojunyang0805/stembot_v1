...
export function StudentOnly({
  ...
  return (
    <AuthGuard requireAuth={true} requireRoles={['student']} fallback={fallback}>
      {children}
    </AuthGuard>
  )
}
...
export function EducatorOnly({
  ...
  return (
    <AuthGuard requireAuth={true} requireRoles={['educator']} fallback={fallback}>
      {children}
    </AuthGuard>
  )
}
...
export function RoleBasedRender({
  ...
  const { isAuthenticated, isStudent, isEducator, isAdmin, isLoading } = useAuth()
